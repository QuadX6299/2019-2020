...
@Config
@Autonomous (name = "1 f pure r", group = "Autonomous")
public class RedAuto1B1F extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while (!isStarted()) {
            String visionPose = nbb.redVision();
        ...
        while (opModeIsActive()) {
            ...
            switch (state) {
                ...
                case FOUNDATION:
                    ...
                    Thread.sleep(400);
                    Robot.driveTrain.turnPID(.4, true, Math.PI);
                ...
                case PARK:
                    List<Waypoint> park = new PathBuilder(new Waypoint(Robot.driveTrain.getPoseEstimate().getX(), Robot.driveTrain.getPoseEstimate().getY(), Math.PI/2.0, 10.0, null))
                            .addPoint(new Waypoint(26, -50, Math.PI, 10.0, new StaticRoutine(() -> {
                                Robot.foundationHooks.down();
                                return Unit.INSTANCE;
                            })))
                            .addPoint(new End(4.0,0.0,new Waypoint(24,-30.0,Math.PI,10.0, null)))
                            .build();
